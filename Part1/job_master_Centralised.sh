#! /bin/bash
if [ -z "$1" ]; then
	echo "Usage $0: requires subdirectory as argument."
	exit 1
fi
if ! [ -d $1 ]; then
...
do
	./map_centralised.sh $file & #start map script in background with file in directory as argument
...
for i in $keys;do #iterate over value in $keys
	...
	./reduce_centralised.sh $i & #start reduce.sh script in the background with value from keys as argument.
done
